...
// Join client via client_id or fall back to name match; COALESCE covers kra_pin → id_number legacy
$stmt = $db->prepare("
    SELECT p.*,
           i.invoice_number, i.id AS inv_id, i.total AS inv_total,
           sb.booking_number, sb.id AS bk_id,
           cl.id_number AS client_id_number,
           COALESCE(NULLIF(TRIM(cl.kra_pin),''), NULLIF(TRIM(cl.id_number),'')) AS display_kra_pin
    FROM payments p
    ...
    LEFT JOIN clients cl ON cl.id = COALESCE(
        ...
        (SELECT id FROM clients
         WHERE LOWER(TRIM(name)) = LOWER(TRIM(p.client_name))
           AND status = 'active'
         LIMIT 1)
    )
    WHERE p.id = ?
...
<div class="receipt-wrapper">
    ...
    <div class="receipt-body">
        ...
        <?php endif; ?>
        <?php if ($p['display_kra_pin']): ?>
        <div class="info-row">
            <span class="label">KRA PIN</span>
            <span class="value" style="font-weight:700"><?= e(strtoupper($p['display_kra_pin'])) ?></span>
        </div>
        <?php endif; ?>

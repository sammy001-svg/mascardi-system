...
// Load linked user account (if any)
$stmt2 = $db->prepare("SELECT * FROM users WHERE linked_type='driver' AND linked_id=? LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $action   = $_POST['account_action'] ?? '';   // 'create' | 'update' | 'disable' | 'enable' | ''
    ...
    if (empty($errors)) {
        try {
            ...
            $errors[] = $e->getCode() === '23000' ? 'ID/License or username already exists.' : $e->getMessage();
        }
    }
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="mb-0">Edit Driver: <?= e($drv['name']) ?></h5>
...
<div class="card">
    <div class="card-body">
        <form method="POST">
            <div class="row g-3">
                <div class="col-md-6"><label class="form-label">Full Name <span class="text-danger">*</span></label><input type="text" name="name" class="form-control" value="<?= e($drv['name']) ?>" required></div>
                <div class="col-md-6"><label class="form-label">Phone <span class="text-danger">*</span></label><input type="text" name="phone" class="form-control" value="<?= e($drv['phone']) ?>"></div>
                <div class="col-md-4"><label class="form-label">National ID <span class="text-danger">*</span></label><input type="text" name="id_number" class="form-control" value="<?= e($drv['id_number']) ?>"></div>
                <div class="col-md-4"><label class="form-label">License Number <span class="text-danger">*</span></label><input type="text" name="license_number" class="form-control" value="<?= e($drv['license_number']) ?>"></div>
                ...
                <div class="col-md-2"><label class="form-label">Status</label>
                    <select name="status" class="form-select">
                        <option value="active"   <?= $drv['status'] === 'active'   ? 'selected' : '' ?>>Active</option>
                        <option value="inactive" <?= $drv['status'] === 'inactive' ? 'selected' : '' ?>>Inactive</option>
                    </select>
                </div>
            ...
            <div class="form-section mt-4">
                ...
                <div class="d-flex align-items-center gap-3 mb-3 p-3 bg-light rounded-3">
                    <div class="user-avatar bg-primary text-white"><?= strtoupper(substr($linkedUser['username'], 0, 1)) ?></div>
                    <div class="flex-grow-1">
                        <div class="fw-semibold"><?= e($linkedUser['username']) ?></div>
                        <div class="small text-muted">Driver account &mdash; <?= statusBadge($linkedUser['status']) ?></div>
                    </div>
                ...
                <div class="alert alert-secondary py-2 small mb-3">
                    <i class="fa fa-circle-info me-2"></i>This driver does not have a login account yet.
                </div>

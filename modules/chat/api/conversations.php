...
if ($method === 'GET') {
    ...
    // Build display name for each conversation
    foreach ($convs as &$c) {
        if ($c['type'] === 'direct') {
            $c['display_name'] = $c['other_user_name'] ?? 'Unknown';
        } else {
            $c['display_name'] = $c['name'] ?? 'Group';
        }
        // Summarise last message for preview
        if ($c['last_msg_type'] === 'voice') {
            $c['last_preview'] = '🎤 Voice note';
        } elseif ($c['last_msg_type'] === 'image') {
            $c['last_preview'] = '📷 Photo';
        } elseif ($c['last_msg_type'] === 'file') {
            $c['last_preview'] = '📎 ' . ($c['last_msg_file'] ?? 'File');
        } elseif ($c['last_msg_type'] === 'call') {
            $c['last_preview'] = '📞 ' . ($c['last_msg_content'] ?? 'Call');
        } elseif ($c['last_msg_type'] === 'system') {
            $c['last_preview'] = $c['last_msg_content'] ?? '';
        } else {
            $c['last_preview'] = $c['last_msg_content'] ?? '';
        }
    }

...
<script>
// Both of these are rendered as hidden inputs rather than dropdowns when the
// user only holds client rights (the type and the client are fixed then), so
// each handler only binds if it actually found a <select> to listen to.
(function () {
    const typeSel = document.getElementById('car_type');
    ...
    if (clientSel && clientSel.tagName === 'SELECT') {
        clientSel.addEventListener('change', function () {
            ...
            if (opt && opt.value && (typeSel ? typeSel.value : '') === 'client') {

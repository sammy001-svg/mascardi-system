...
// Approvals waiting on this admin. index.php redirects 'admin' straight here, so
// these banners must live on this dashboard — otherwise a top-level approver never
// sees anything actionable and reservations stall at "awaiting approval".
require_once __DIR__ . '/../../includes/notifications.php';
$dpPendingItems = getPendingDeliveryProtocolActions(authUser()['role'] ?? '');
$pendingReservations = [];
try {
    $pendingReservations = $db->query("
        SELECT cl.id, cl.name, cl.deposit_amount, cl.agreed_sale_price,
    ...
    <div class="card-body" style="background:#fffbeb">
        ...
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 py-2"
            ...
            <div class="d-flex gap-2">
                <a href="<?= BASE_URL ?>/modules/crm/view_lead.php?id=<?= (int)$r['id'] ?>&dp_open=1"
                   class="btn btn-sm btn-outline-secondary"><i class="fa fa-eye me-1"></i>Review</a>

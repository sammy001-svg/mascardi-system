...
// Flatten into indexed array so numeric keys align across all POST arrays
$allParts = [];
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="mb-0"><i class="fa fa-clipboard-check me-2 text-primary"></i>New Car Assessment</h5>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    <?php
                    $types = ['arrival' => ['Arrival', 'fa-anchor'], 'workshop' => ['Workshop', 'fa-toolbox'], 'pre_delivery' => ['Pre-Delivery', 'fa-flag-checkered']];
                    foreach ($types as $val => [$lbl, $icon]):
                    ?>
                    <div class="type-option">
                        <input type="radio" name="assessment_type" id="type_<?= $val ?>" value="<?= $val ?>"
                            class="type-radio" <?= $postType === $val ? 'checked' : '' ?>>
                        <label for="type_<?= $val ?>" class="type-btn">
            ...
            <div class="col-md-4">
                ...
                <div class="d-flex gap-2 flex-wrap" id="overallStatusGroup">
                    <?php
                    ...
                    <div>
                        ...
                        <label for="os_<?= $val ?>" class="os-btn" style="--os-color:<?= $color ?>">
                            <?= $lbl ?>
                        </label>
                    </div>
                    <?php endforeach; ?>
            ...
            <div class="col-md-2">
                ...
                <input type="number" name="mileage" class="form-control" min="0"
                    value="<?= e($postMileage) ?>" placeholder="e.g. 45000">
            </div>
            ...
            <div class="col-md-4">
                ...
                <div class="fuel-gauge">
                    <?php
                    $fuelOpts = ['empty' => 'E', 'quarter' => '¼', 'half' => '½', 'three_quarter' => '¾', 'full' => 'F'];
            ...
            <div class="col-12">
                ...
                <textarea name="notes" class="form-control" rows="2" placeholder="Overall observations about the vehicle..."><?= e($postNotes) ?></textarea>
            </div>
...
$idx = 0;
foreach ($partsList as $category => $parts):
    $catSlug = preg_replace('/[^a-z0-9]/i', '_', $category);
    $catIcons = [
...
<div class="assess-category mb-3" data-category="<?= e($category) ?>">
    ...
    <div class="assess-cat-body">
        ...
        <div class="assess-part-row" data-idx="<?= $idx ?>">
            ...
            <!-- Notes (shown only when condition != good) -->
...
function onCondChange(radio) {
    ...
    var isGood = radio.value === 'good';
    if (notes) notes.style.display = isGood ? 'none' : '';
    updateCounts();
}
...
function updateCounts() {
    ...
    if (issues > 0) {
        gb.textContent = issues + ' issue' + (issues > 1 ? 's' : '');
    ...
    document.querySelectorAll('.assess-category').forEach(function (cat) {
        var catRows   = cat.querySelectorAll('.assess-part-row');
        var catIssues = 0;
        ...
        if (badge) {
            if (catIssues > 0) {
                badge.textContent = catIssues + ' issue' + (catIssues > 1 ? 's' : '');
    ...
    var suggested = ['excellent', 'good', 'fair', 'poor', 'critical'];
    var osMap = {0:'excellent', 1:'good', 2:'fair', 3:'poor', 4:'critical'};
    var suggestId = 'os_' + (osMap[worstVal] || 'fair');
    document.querySelectorAll('.os-radio').forEach(function (r) {
        r.dataset.suggested = '';
    });
    var sr = document.getElementById(suggestId);
    if (sr && !document.querySelector('.os-radio:checked[data-user-set]')) {
        sr.checked = true;
    }
}
...
/* ── Mark whole category good ─────────────────────────── */
function markCatGood(btn) {
    var cat = btn.closest('.assess-category');
    cat.querySelectorAll('.assess-part-row').forEach(function (row) {
        var goodRadio = row.querySelector('input[value="good"]');
        if (goodRadio) {
            goodRadio.checked = true;
            var notes = row.querySelector('.assess-part-notes');
...
/* ── Mark ALL parts good ──────────────────────────────── */
document.getElementById('markAllGoodBtn').addEventListener('click', function () {
    document.querySelectorAll('input[type=radio][value="good"]').forEach(function (r) {
        if (r.name.startsWith('part_condition')) r.checked = true;
    });
    document.querySelectorAll('.assess-part-notes').forEach(function (n) {
        n.style.display = 'none';
    });
    updateCounts();
});

/* ── Track when user manually sets overall status ─────── */
document.querySelectorAll('.os-radio').forEach(function (r) {
